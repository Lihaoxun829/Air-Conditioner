...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        print(f"[APP] Raw message received: {msg.payload.decode('utf-8')}")
        ...
        # Exit immediately if shutdown message is detected
        if data['ac_status'] == "OFF" and "shut down" in data['message'].lower():
            print("[APP] Received shutdown instruction. Exiting...")
            running = False
            client.loop_stop()
            client.disconnect()
            return

        # Otherwise, count message and exit after 5
        message_count += 1
...
# Keep running until shutdown or max message count
